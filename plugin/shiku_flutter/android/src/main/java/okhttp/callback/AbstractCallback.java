...
//                String realData = LoginSecureHelper.decodeLoginResult(code, body.getData().getData());

                successData(parseResponse(call, body));
            } catch (Exception e) {
                com.sk.weichat.util.LogUtils.log(response);
    ...
    private void callOnResponse(T t) {
        if (t instanceof Result) {
            ...
            Log.d(TAG, "resultCode哈哈哈哈:" + resultCode);

            if (resultCode == Result.CODE_TOKEN_ERROR) {
                Log.d(TAG, "CODE_TOKEN_ERROR:" + resultCode);
...
//                LoginHelper.broadcastLogout(MyApplication.getContext());
        ...
        Log.d(TAG, "resultCode哈哈哈哈:开始onResponse" + t);
